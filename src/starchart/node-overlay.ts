 ...
 * 콘텐츠가 노드 상세에 얹히는 경로가 여기다. 주요 노드 본문 저작은 스펙 이후
 ...
 * 같은 이유·같은 규약이다.
 *
 * 기준 패치는 파일이 아니라 노드마다 있다. 주요 노드는 한 번에 쓰이지 않고 한
 ...
 *
 * 목록을 따로 적지 않고 데이터셋에서 파생한다. 손으로 적은 목록은 게임 패치로
 * 노드가 늘거나 줄 때 조용히 어긋나고, 그 어긋남은 "왜 이 노드엔 오버레이를 못
 * 붙이지"라는 모양으로만 드러난다. 지금 데이터셋 기준으로 교차점 13 + 보스 20 =
 * 33이며, 스펙 §3.3이 적은 "교차점 12"와 갈리면 데이터셋이 사실이다.
 */
