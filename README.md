...
e2e에 남기는 기준은 하나다 — **다른 층에서 볼 수 없는 것**. WebGL 기동과 폴백 전환,
카메라 연속 비행, 마우스·키보드·터치·뷰포트·새 탭·localStorage 같은 브라우저 고유
동작, 그리고 그것들을 잇는 최소 경로. 완료 집합의 파생 규칙, 패널이 무엇을 보여
주는가, 배너가 언제 뜨는가 같은 **규칙**은 유닛·컴포넌트 테스트(`src/**/*.test.ts(x)`)가
맡는다. e2e 한 건은 3D 씬을 띄우는 값(CI 러너에서 5~15초)을 치르므로, 같은 단언이
아래층에 이미 있으면 e2e에 두지 않는다.
