...
- **전환은 "연속 비행"**: 컷 전환·모프가 아니라, 한 장면 안에서 카메라가 행성까지
  ...
  않는 연속성이 핵심 경험이다. 사용자가 모션 감소(`prefers-reduced-motion`)를 켰으면
  보간 없이 목적지에 바로 선다 — 도착지는 같다(#72).
...
- 표시명 오버라이드: 사전이 키를 오역하는 경우(누락이 아니다)만 빌드 스크립트의
  표 하나로 덮는다. 그 키가 더 이상 쓰이지 않거나 사전이 우리 표기와 같아지면
  빌드 실패 — 예외가 조용히 남지 않는다.
...
- 사용자가 완료한 id의 집합 하나. 성계 노드(`SolNodeNN`)와 비노드 목표(퀘스트
  + 준비 목표 슬러그, 잠정 목록)가 공존. 교차점은 SolNode id로만 기록.
  ([#49](https://github.com/b-wani/warframe-hub/issues/49)에서 그릇을 세우며
  옛 로드맵의 항목을 옮겨 보니 퀘스트 8 + 준비 1 = 9였다 — 여기 적혀 있던
  "퀘스트 7"은 교차점 7과 자릿수가 엇갈린 셈으로 보인다. 목록의 확정 재선정은
  저작 단계 소관이다 — §9.)
...
근거: [#34 모바일 정책](https://github.com/b-wani/warframe-hub/issues/34), [#41 성계 뷰 골격](https://github.com/b-wani/warframe-hub/issues/41) (적응 규칙 1 종횡비 인지 프레이밍 구현 완료), [#51 모바일 적응 규칙](https://github.com/b-wani/warframe-hub/issues/51) (적응 규칙 2·3·4 구현 완료 — 호버 의존 없음 점검, 노드·천체 히트 영역을 화면 44px 기준으로 키움(이웃 중심을 삼키지 않는 선까지), 좁은 폭 HUD 띠 배치. 중급 안드로이드 실기기 스모크는 미실시), [#45 폴백 뷰](https://github.com/b-wani/warframe-hub/issues/45) (WebGL 불가 시 자동 표시·수동 전환·행성 아코디언 체크리스트 구현 완료)
...
- 행성 가이드 저작 워크플로·범위, 비노드 목표 행성별 확정 재선정, 큐레이션
  ...
  + 라이노 준비 1)이며, 저작 단계가 재선정한다. 가이드는 저작된 천체에만 선다 —
  빈 패널을 모든 행성에 세워 두지 않는다.
